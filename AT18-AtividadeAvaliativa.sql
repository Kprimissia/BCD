...
SELECT metros_distancia,
COUNT(id_corredor) AS numero_corredores
FROM tb_corredor
GROUP BY metros_distancia
HAVING COUNT(id_corredor) > 3;
...
MAX(dt_evento) AS data_mais_recente
FROM tb_corredor
JOIN tb_corredor_evento ON tb_corredor.id_corredor = tb_corredor_evento.CORREDOR_id_corredor
JOIN tb_evento ON tb_corredor_evento.EVENTO_id_evento = tb_evento.id_evento
WHERE dt_nasc = (SELECT MAX(dt_nasc) 
FROM tb_corredor JOIN tb_corredor_evento ON tb_corredor.id_corredor = tb_corredor_evento.CORREDOR_id_corredor)
GROUP BY nome_corredor;
...
SELECT nome_corredor, MAX(dt_evento) AS data_mais_recente
FROM tb_corredor
JOIN tb_corredor_evento ON tb_corredor.id_corredor = tb_corredor_evento.CORREDOR_id_corredor
JOIN tb_evento ON tb_corredor_evento.EVENTO_id_evento = tb_evento.id_evento
WHERE dt_nasc = (SELECT MAX(dt_nasc) 
